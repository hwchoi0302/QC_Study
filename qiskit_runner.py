...
class QuantumManager:
    ...
    def __init__(self, run_mode="simulator", backend_name=None, optimization_level=None):
        ...
        if self.run_mode == "simulator":
            ...
            if optimization_level is None:
                optimization_level = 1  # 시뮬레이터는 최적화 레벨을 낮춰 클래식 연산 자원 절약
            ...
            if optimization_level is None:
                optimization_level = 3  # 실제 장비는 노이즈 최소화를 위해 최고 수준의 최적화 적용
        ...
        print(f"[{self.run_mode.upper()} MODE] 연결된 백엔드: {self.backend.name} (Opt Level: {optimization_level})")
    ...
    def run(self, qc, primitive="sampler", observables=None, shots=1024):
        ...
        if primitive == "sampler":
            ...
            if observables is None:
                raise ValueError("estimator를 사용할 경우 observables를 제공해야 합니다.")
            ...
            raise ValueError(f"지원하지 않는 primitive 입니다: {primitive}. 'sampler' 또는 'estimator'를 사용하세요.")
